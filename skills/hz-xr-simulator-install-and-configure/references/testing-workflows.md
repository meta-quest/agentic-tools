...
- [Scriptable testing](#scriptable-testing)
- [Debugging with XR Simulator](#debugging-with-xr-simulator)
- [Limitations to be aware of](#limitations-to-be-aware-of)
- [When to test on a physical device](#when-to-test-on-a-physical-device)
...
- **Performance characteristics differ from Meta VR hardware**. The simulator runs on a desktop CPU and GPU. Frame times, draw call budgets, and memory limits do not match Meta VR hardware. Never use simulator results for performance optimization decisions.
...
- **Thermal throttling does not apply**. The device dynamically adjusts performance based on thermal state. The simulator does not simulate this behavior.
- **Audio spatialization may differ**. The simulator outputs audio through desktop speakers or headphones. HRTF and spatial audio behavior may not match the device's built-in audio pipeline.
...
- **Final performance validation**: Confirm frame rate, GPU utilization, and memory usage on Meta VR hardware.
