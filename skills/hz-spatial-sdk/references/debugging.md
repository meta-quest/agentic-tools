...
This reference covers debugging tools and techniques for Meta Spatial SDK applications, including the Data Model Inspector, OVR Metrics Tool, logcat filtering, metavr integration, and common issue diagnosis.
...
metavr adb shell setprop debug.oculus.perf.level 1
```
...
metavr log | grep "SpatialSDK\|Panel\|Scene\|DataModel"

# View only errors
metavr log --level E

# View panel-specific messages
metavr adb logcat --tag Panel

# View scene loading messages
metavr adb logcat --tag Scene --level I

# View ECS system execution issues
metavr adb logcat --tag ECS --level W
```
...
metavr capture screenshot --output ./debug_screenshot.png
```
...
metavr perf load <session_id>
```

## metavr Usage for Debugging

The `metavr` CLI provides essential debugging commands for Spatial SDK development.
...
metavr app stop com.example.myspatialapp
metavr app launch com.example.myspatialapp

# Clear app data (reset to fresh state)
metavr app clear com.example.myspatialapp
```
...
4. Check for Compose or View exceptions in the logs:
   ```bash
   metavr log --level E | grep -i "compose\|view\|layout\|render"
   ```
...
1. Verify the glTF file path is correct and the file is included in the APK assets:
   ```bash
   metavr log | grep -i "mesh\|glb\|gltf\|asset\|load"
   ```
...
4. Check for mesh loading errors in the logs:
   ```bash
   metavr log --level E | grep -i "mesh\|load\|file\|uri"
   ```
...
4. Check for physics errors in the logs:
   ```bash
   metavr log | grep -i "physics\|collision\|rigidbody"
   ```
