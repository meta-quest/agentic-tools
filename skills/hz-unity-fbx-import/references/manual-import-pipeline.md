# Manual FBX Import Pipeline

Use this when you need the full one-shot import behavior (download, scale-to-height, material
assignment, prefab creation) rather than the plain `unity command import_asset` flow in
[SKILL.md](../SKILL.md) — and `Unity_ImportExternalModel` is unavailable.

The reference implementation lives in the `com.unity.ai.assistant` package:

**Package:** `com.unity.ai.assistant`
**Path within package:** `Modules/Unity.AI.MCP.Editor/Tools/ImportExternalModel.cs`

Read this file before implementing. The key method is `DownloadAndImportModelInScene()` which orchestrates the full pipeline. Replicate its logic across **separate calls** so failures are isolated and debuggable.

Under `unity-cli` each step is a `run_script` invocation with a plain static entry point; under Unity
MCP each is a `Unity_RunCommand` script wrapped in `CommandScript : IRunCommand`. See
[unity-mcp-fallback.md](unity-mcp-fallback.md) for the harness differences.
...
Execute the five steps **sequentially via separate calls**, so a failure is attributable to one step.
...
After Step 5, report the import directory, GameObject data (name, instance ID, bounds), prefab path, and prefab data.

Step 2 writes into `Assets/`, which triggers an asset import — so steps 3-5 must be separate calls
rather than chained into the same script, or they will run against a stale `AssetDatabase`.
