...
| Option | Description |
|--------|-------------|
| `-d, --device <DEVICE>` | Target device ID (overrides HZDB_DEVICE) |
...
| `-d, --disable` | Disable the proximity sensor (keeps headset awake regardless of wear) |
...
| Command | Description |
|---------|-------------|
| [`capture`](#capture) | Capture a Perfetto trace from a connected device |
...
Capture a Perfetto trace from a connected device
...
| `--gpu-metrics` | Enable GPU metrics tracing (default: true) |
| `--cpu-scheduling` | Enable CPU scheduling tracing (default: true) |
